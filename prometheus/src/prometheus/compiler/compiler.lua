...
function Compiler:compile(ast)
    ...
    self.registerVars = {};
    self.usedBlockIds = {};
    -- Per-build register ID permutation (instruction field shuffling)
    -- Maps logical register id → permuted id, like IronBrew InstructionSteps shuffle
    do
        local perm = {}
        for i = 0, MAX_REGS - 1 do perm[i] = i end
        for i = MAX_REGS - 1, 1, -1 do
            local j = math.random(0, i)
            perm[i], perm[j] = perm[j], perm[i]
        end
        self.regPerm = perm
    end
    -- Per-build random offset: all emitted block IDs = raw_id + offset
    ...
    self.uvEncVar = self.scope:addVariable();
    self.uvDecVar = self.scope:addVariable();
    self.scAliasVar = self.scope:addVariable();  -- string.char local alias (faster lookup)
    ...
    local functionNodeAssignments = {
        {
            ...
            var = Ast.AssignmentVariable(self.scope, self.uvEncVar),
            val = self:createUvEncFunc(),
        }, {
            var = Ast.AssignmentVariable(self.scope, self.uvDecVar),
            val = self:createUvDecFunc(),
        }, {
            var = Ast.AssignmentVariable(self.scope, self.scAliasVar),
            val = Ast.IndexExpression(
                Ast.IndexExpression(self:env(self.scope), Ast.StringExpression("string")),
    ...
    local tbl = {
        Ast.VariableExpression(self.scope, self.scAliasVar),
        Ast.VariableExpression(self.scope, self.uvEncVar),
        Ast.VariableExpression(self.scope, self.uvDecVar),
...
function Compiler:createAllocUpvalFunction()
    ...
    -- Técnica 2: non-sequential upvalue IDs via step increment
    local uvStep = math.random(2, 7);  -- skip slots to make IDs non-sequential
...
function Compiler:emitContainerFuncBody()
    ...
    -- Técnica 5: inline cache for common constants (env["string"], env["type"] etc)
    local envStringVar = self.containerFuncScope:addVariable();
    local envTypeVar   = self.containerFuncScope:addVariable();
    table.insert(declarations, envStringVar);
    table.insert(declarations, envTypeVar);

    local stats = {
        Ast.LocalVariableDeclaration(self.containerFuncScope, util.shuffle(declarations), {});
        -- Pre-load common globals into locals for O(1) access inside VM
        Ast.AssignmentStatement(
            {Ast.AssignmentVariable(self.containerFuncScope, envStringVar)},
            {Ast.IndexExpression(self:env(self.containerFuncScope), Ast.StringExpression("string"))}
        );
        Ast.AssignmentStatement(
            {Ast.AssignmentVariable(self.containerFuncScope, envTypeVar)},
            {Ast.IndexExpression(self:env(self.containerFuncScope), Ast.StringExpression("type"))}
        );
...
function Compiler:getRegisterVarId(id)
    -- Apply per-build permutation to register id
    local permId = (self.regPerm and id < MAX_REGS) and self.regPerm[id] or id;
    local varId = self.registerVars[permId];
    if not varId then
        varId = self.containerFuncScope:addVariable();
        self.registerVars[permId] = varId;
    end
    return varId;
end
...
function Compiler:setPos(scope, val)
    ...
    if math.random(1, 2) == 1 then
        idExpr = Ast.AddExpression(
            Ast.SubExpression(Ast.NumberExpression(emittedVal + noise), Ast.NumberExpression(noise)),
            Ast.MulExpression(Ast.NumberExpression(noise), Ast.NumberExpression(0))
        );
    else
        idExpr = Ast.AddExpression(Ast.NumberExpression(emittedVal - noise), Ast.NumberExpression(noise));
    end
    return Ast.AssignmentStatement({Ast.AssignmentVariable(self.containerFuncScope, self.posVar)}, {idExpr});
end
...
function Compiler:setUpvalueMember(scope, idExpr, valExpr, compoundConstructor)
    ...
    if compoundConstructor then
        return compoundConstructor(Ast.AssignmentIndexing(Ast.VariableExpression(self.scope, self.upvaluesTable), scrambledIdx), valExpr);
    end
    return Ast.AssignmentStatement({Ast.AssignmentIndexing(Ast.VariableExpression(self.scope, self.upvaluesTable), scrambledIdx)}, {valExpr});
end
...
function Compiler:getUpvalueMember(scope, idExpr)
    ...
    return Ast.IndexExpression(Ast.VariableExpression(self.scope, self.upvaluesTable), scrambledIdx);
end
...
-- Técnica 4: Anti-dump sentinel injected into VM scope
function Compiler:createAntiDumpSentinel()
    local scope = Scope:new(self.scope);
    scope:addReferenceToHigherScope(self.scope, self.envVar);
    local vSelf = scope:addVariable();
    -- setmetatable({}, {__gc = function() env["print"] = nil; env["warn"] = nil end})
    -- When the VM is dumped, GC collects the sentinel and poisons env
    local gcScope = Scope:new(scope);
    gcScope:addReferenceToHigherScope(self.scope, self.envVar);
    local gcFn = Ast.FunctionLiteralExpression({}, Ast.Block({
        Ast.AssignmentStatement(
            {Ast.AssignmentIndexing(
                self:env(gcScope),
                Ast.StrCatExpression(Ast.StringExpression("pr"), Ast.StringExpression("int"))
            )},
            {Ast.NilExpression()}
        ),
        Ast.AssignmentStatement(
            {Ast.AssignmentIndexing(
                self:env(gcScope),
                Ast.StrCatExpression(Ast.StringExpression("wa"), Ast.StringExpression("rn"))
            )},
            {Ast.NilExpression()}
        ),
    }, gcScope));
    local mtTbl = Ast.TableConstructorExpression({
        Ast.KeyedTableEntry(Ast.StringExpression("__gc"), gcFn)
    });
    return Ast.FunctionCallExpression(
        Ast.VariableExpression(self.scope, self.setmetatableVar),
        {Ast.TableConstructorExpression({}), mtTbl}
    );
end

-- Upvalue encryption (additive cipher, numbers only)
function Compiler:createUvEncFunc()
    local scope = Scope:new(self.scope);
    local vArg = scope:addVariable();
    local vKey = self.uvKey or 0;
    -- function(v) if type(v)=="number" then return v + KEY else return v end end
    local ifScope = Scope:new(scope);
    return Ast.FunctionLiteralExpression({Ast.VariableExpression(scope, vArg)}, Ast.Block({
        Ast.IfStatement(
            Ast.EqualsExpression(
                Ast.FunctionCallExpression(
                    Ast.VariableExpression(scope:resolveGlobal("type")),
                    {Ast.VariableExpression(scope, vArg)}
                ),
                Ast.StringExpression("number")
            ),
            Ast.Block({
                Ast.ReturnStatement({
                    Ast.AddExpression(Ast.VariableExpression(scope, vArg), Ast.NumberExpression(vKey))
                })
            }, ifScope),
            {}, nil
        ),
        Ast.ReturnStatement({Ast.VariableExpression(scope, vArg)})
    }, scope));
end

function Compiler:createUvDecFunc()
    local scope = Scope:new(self.scope);
    local vArg = scope:addVariable();
    local vKey = self.uvKey or 0;
    -- function(v) if type(v)=="number" then return v - KEY else return v end end
    ...
    return Ast.FunctionLiteralExpression({Ast.VariableExpression(scope, vArg)}, Ast.Block({
        Ast.IfStatement(
            ...
            Ast.Block({
                Ast.ReturnStatement({
                    Ast.SubExpression(Ast.VariableExpression(scope, vArg), Ast.NumberExpression(vKey))
                })
            }, ifScope),
            {}, nil
        ),
