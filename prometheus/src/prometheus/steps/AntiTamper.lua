...
-- Frag3: getfenv, debug.getinfo, os.clock timing, debug.traceback format
    ...
    -- Frag4: RunService identity + ScriptContext check
    -- Deobfuscators run outside Roblox: RunService/ScriptContext don't exist or behave wrong
    local frag4 = [==[
    do
        local _rg4  = rawget
        ...
        -- Check A: RunService IsClient/IsServer consistency
        local _ok4a, _rs4 = _pc4(function()
            return game:GetService("Run".."Service")
        end)
        if _ok4a and _rs4 then
            local _ok4b, _ic4 = _pc4(function() return _rs4:Is".."Client() end)
            local _ok4c, _is4 = _pc4(function() return _rs4:Is".."Server() end)
            -- In Roblox: IsClient XOR IsServer is always true
            -- In deobf: both might return false or same value
            if _ok4b and _ok4c then
                if _ic4 == _is4 then  -- both true or both false = fake env
        ...
        -- Check B: game.PlaceId must be a positive integer
        ...
        -- Check C: tostring(game) must contain "DataModel"
        -- Deobf environments return generic object strings
